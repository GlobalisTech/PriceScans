...
# PARUL_PORTFOLIO = [
#     "AIRTELPP", "BHARTIARTL", "CGPOWER", "JSWENERGY", "M&M", "POLYMED", 
#     "SUZLON", "TITAGARH", "ZOMATO"]
...
# "GRAPHITE", "HBLENGINE", "HEG", "JCHAC", "JSLL", "MCX", "MOS", 
# "OLECTRA", "PAR", "PASHUPATI", "POCL", "POKARNA", "PPLPHARMA", 
...
ONEDRIVE_FILE_URL = "https://1drv.ms/x/c/b57d0a53c133130b/EdGZmCnFy7ZAlMm1-xp8hTsBLSDQgbJuZTDqoc6l0Dt1dg?e=pyikXb&nav=MTVfe0FENEMzNjY0LTY4RjItNDNGOC1CREVDLTFDREQ2MEM0QTdFMn0"
...
def download_from_onedrive(url, filename):
    try:
        ...
        if response.status_code == 200:
            ...
            st.error(f"Failed to download file from OneDrive. Status code: {response.status_code}")
            return None
    except Exception as e:
        st.error(f"Error downloading file: {str(e)}")
        return None
...
def main():
    ...
    downloaded_file = download_from_onedrive(ONEDRIVE_FILE_URL, FILE_NAME)
    if downloaded_file is None:
        st.error("Failed to download the file. Please check the OneDrive link and try again.")
        return
